...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv2d(dim, dim, kernel_size=3,
                                 padding=p, bias=use_bias), nn.ReLU(True)]
        if use_dropout:
...
class SSIMLoss(torch.nn.Module):
    ...
    def forward(self, pred, target):
        ...
        loss = 1.0 - self.ssim(pred, target)
        return loss
...
class VGGLoss(nn.Module):
    def __init__(self, device, vgg=None, weights=None, indices=None, normalize=True):
        ...
        self.weights = weights or [0.4, 0.2, 0.82, 0.73]
        self.indices = indices or [2, 7, 21, 30]
        self.device = device
        if normalize:
            self.normalize = MeanShift([0.485, 0.456, 0.406], [
                                       0.229, 0.224, 0.225], norm=True).to(self.device)
        else:
            self.normalize = None
        print("Vgg: Weights: ", self.weights, " indices: ",
              self.indices, " normalize: ", self.normalize)
...
class DeConvLayer(torch.nn.Sequential):
    def __init__(self, deconv, in_channels, out_channels, kernel_size, stride, padding=None, dilation=1, norm=None,
        ...
        self.add_module('TransposeConv2d', deconv(in_channels, out_channels, kernel_size,
                        stride, padding=padding, output_padding=output_padding, dilation=dilation))
...
class ConvLayer(torch.nn.Sequential):
    def __init__(self, conv, in_channels, out_channels, kernel_size, stride, padding=None, dilation=1, norm=None,
        ...
        if padding is None:  # auto cal
            ...
            self.add_module('padding', nn.ReflectionPad2d(
                (paddingL, paddingR, paddingL, paddingR)))
...
# ''' We use similar synthetic model as Zhang's(Single image reflection separation with perceptual losses.)'''
...
class Generator_drop(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n_feats):
        ...
        self.cbam_e2 = CBAM(192, ratio=32)
        self.rstb_e3 = RSTB(
            img_size=64,
            in_chans=192,
            embed_dim=96,
