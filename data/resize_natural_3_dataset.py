...
class ResizeNatural3Dataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.phase == 'train':
            ...
            self.natural_A1_paths = sorted(make_dataset(
                self.natural_dir_A1, opt.max_dataset_size))
            # load images from '/path/to/data/trainB'
            self.natural_B_paths = sorted(make_dataset(
                self.natural_dir_B, opt.max_dataset_size))
            # get the size of dataset A1
            self.natural_size = len(self.natural_A1_paths)
    ...
    def __getitem__(self, index):
        ...
        if self.opt.phase == 'train':
            if self.opt.use_all_natural:
                if index < self.natural_size:
                    # Load full natural samples first
                    A1_path = self.natural_A1_paths[index]
                    B_path = self.natural_B_paths[index]
                    ...
                    A1_img, B_img = Image.fromarray(
                        imgs['T']), Image.fromarray(imgs['I'])
                    is_natural_int = 1
                else:
                    # Load synthetic after natural
                    syn_index = index - self.natural_size
                    A1_path = self.A1_paths[syn_index % self.A1_size]
                    A2_path = self.A2_paths[random.randint(
                        0, self.A2_size - 1)]
                    B_path = ''
                ...
                # Probabilistic natural sampling (30%)
    ...
    def __len__(self):
        ...
        if self.opt.dataset_size == 0 or self.opt.phase in ['test', 'val']:
