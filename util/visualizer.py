...
class Visualizer():
    ...
    def __init__(self, opt):
        ...
        # Initialize the log file with headers if it's new
        if not os.path.exists(self.log_name):
            with open(self.log_name, "w") as log_file:
                # We'll add loss columns when we first get losses
                log_file.write('epoch,iter,time,data_time\n')
    ...
    def print_current_losses(self, epoch, iters, losses, t_comp, t_data):
        ...
        with open(self.log_name, "a") as log_file:
            # If this is the first time writing losses, add the loss columns to the header
            if os.path.getsize(self.log_name) == 0:
                loss_columns = ','.join(['loss_' + k for k in losses.keys()])
                log_file.write('epoch,iter,time,data_time,' +
                               loss_columns + '\n')

            # Create CSV row with epoch, iter, time, data_time, and all loss values
            csv_row = [str(epoch), str(iters),
                       f"{t_comp:.3f}", f"{t_data:.3f}"]
            for k in losses.keys():
                csv_row.append(f"{losses.get(k, 0.0):.3f}")
