...
def test(model, opt, test_dataloader, webpage):
    ...
    with torch.no_grad():
        pbar = tqdm(test_dataloader, desc='Testing Reflection')
        for data in pbar:
            i += 1
            ...
            # test reflection
            fake = model.fake_R.clamp(0, 1)
            # load reflection image instead of transmission image
...
def main():
    ...
    print(f'\nTest Reflection SSIM: {test_metrics["test_ssim"]:.4f}')
    print(f'Test Reflection PSNR: {test_metrics["test_psnr"]:.4f}')
    print(f'Test Reflection LPIPS: {test_metrics["test_lpips"]:.4f}')

    # create or apend to log file the losses and metrics (MODIFIED FILENAMES)
    test_loss_log_file = os.path.join(
        opt.results_dir, opt.name, 'test_loss_R_log.txt')
    test_metric_log_file = os.path.join(
        opt.results_dir, opt.name, 'test_metric_R_log.txt')
